...
int main(int argc, char **argv)
{
	...
	int w;
	//per dest queues (need to share between threads)
	for (w = 0; w < veth_port_count; w++)
	{
		for (i = 0; i < 3; i++)
		{
			ring_array[w][i] = ringbuf_create(MAX_BURST_TX_OBJS);
			if (ring_array[w][i] == NULL) 
			{
				printf("ring_array [%d] [%d] is NULL \n", w,i);
			}
		}
	}
	...
	for (i = 0; i < 2; i++)
	{
		...
		{				
			...
			for (w = 0; w < veth_port_count; w++)
			{
				for (k = 0; k < 3; k++)
				{
					t->ring_bf_array[w][k] = ring_array[w][k];
				}
				t->burst_tx_queue_array[w] = burst_tx_queue_veth[w];
			}
		}
		else if (i == 1)
		{	
	...
	//VETH RX THREADS
	int veth_rx_threads_start_index = 2;
	int veth_rx_threads_end_point = 2 + veth_port_count;
	int h = 0, v = 0;
	...
	//VETH TX THREADS
	int veth_tx_threads_start_index = 2 + veth_port_count;
	start_index_for_veth_ports  = n_nic_ports;
	// int k = 0; 
	int m = 0;
	//veth tx threads
	...
	//START NIC RX and NIC TX THREADS
	for (i = 0; i < 2; i++)
	{
		int status;
		if (i == 0)
