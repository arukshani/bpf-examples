...
thread_func_veth_to_nic_tx(void *arg)
{
	...
	while (!t->quit)
	{
		...
		if (ring_buff[0] != NULL)
		{
			...
			{
                // printf("ring_buff 0 is NOT empty \n");
				void *obj2;
			    ...
			    // bcache_prod(port_tx->bc, btx_collector->addr[btx_index]);
		...
		if (ring_buff[1] != NULL)
		{
			...
			{
                // printf("ring_buff 1 is NOT empty \n");
				void *obj2;
		...
		if (ring_buff[2] != NULL)
		{
			...
			{
                // printf("ring_buff 2 is NOT empty \n");
				void *obj2;
	...
	while (!t->quit)
	{
        ...
        if (!ringbuf_is_empty(burst_tx_queue))
		{
            ...
            if (!n_pkts) 
            {
                ...
                // printf("no packets from veth rx\n");
                continue;
            }
	...
	{ // A hack to get the thread to return
		...
		if (veth_side_queue[track_veth_tx_port] != NULL)
		{
			...
			while ((!ringbuf_is_empty(veth_side_queue[track_veth_tx_port])) && (btx_index < MAX_BURST_TX))
			{
				...
				if (burst_tx_queue[track_veth_tx_port] != NULL)
				{
					btx->addr[0] = 0;
					btx->len[0] = 0;
					ringbuf_sp_enqueue(burst_tx_queue[track_veth_tx_port], btx);
				} else {
					printf("burst_tx_queue is NULL \n");
				}
				// free(btx);
...
thread_func_nic(void *arg)
{
	...
	for (w = 0; w < veth_port_count; w++)
	{
		...
		for (x = 0; x < MAX_BURST_TX_OBJS; x++)
		{
			if (!ringbuf_is_empty(burst_tx_queue[w]))
			{
				void *obj;
				ringbuf_sc_dequeue(burst_tx_queue[w], &obj);
				free(obj);
			}
		}
	}
...
int main(int argc, char **argv)
{
	...
	// local queues
	ring_array[0] = ringbuf_create(131072);
	ring_array[1] = ringbuf_create(131072);
	ring_array[2] = ringbuf_create(131072);
	...
	burst_tx_queue_veth = ringbuf_create(MAX_BURST_TX_OBJS); //forward queue
	...
	for (i = 0; i < 3; i++)
	{
		...
		// 		t->burst_tx_queue_array[x] = burst_tx_queue_nic[x];
        //     }
