...
int main(int argc, char **argv)
{
    ...
    int veth_rx_threads_start_index = 0;
	// int veth_rx_threads_end_point = total_nic_threads + veth_port_count;
    int veth_rx_threads_end_point = veth_port_count;
	int h = 0, v = 0;
	...
	start_index_for_veth_ports  = n_nic_ports;
    int veth_tx_threads_end_point = veth_port_count * 2;
	// int k = 0; 
	int m = 0;
	//veth tx threads
	// for (i = veth_tx_threads_start_index; i < n_threads; i++)
    for (i = veth_tx_threads_start_index; i < veth_tx_threads_end_point; i++)
	{
		struct thread_data *t = &thread_data[i];
    ...
    /* NIC TX Threads. */
    int nic_tx_thread_count = (veth_port_count * 2 ) + n_nic_ports;
    ...
    int nic_port_index=0;
	for (i = veth_tx_threads_end_point; i < nic_tx_thread_count; i++)
	{
		struct thread_data *t = &thread_data[i];
		
        // veth->nic (nic tx: pull from local and non-local)
        t->ports_tx[0] = ports[nic_port_index];
        nic_port_index++;
    ...
    int j = 0;
	for (i = nic_tx_thread_count; i < n_threads; i++)
	{
		struct thread_data *t = &thread_data[i];
	...
	for (i = 0; i < veth_rx_threads_end_point; i++)
	{
		int status;
	...
	for (i = veth_rx_threads_end_point; i < veth_tx_threads_end_point; i++)
	{
		int status;
		status = pthread_create(&threads[i],
								NULL,
