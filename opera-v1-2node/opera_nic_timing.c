...
clockid_t clkid;
unsigned long total_tx_aloc_ns = 0;
unsigned long total_tx_send1_ns = 0;
unsigned long total_tx_send2_ns = 0;
...
{
	struct timespec start_tx_aloc_time = get_realtime();
	unsigned long start_tx_aloc_ns = get_nsec(&start_tx_aloc_time);
	...
	struct timespec end_tx_aloc_time = get_realtime();
	unsigned long end_tx_aloc_ns = get_nsec(&end_tx_aloc_time);
	unsigned long delta_tx_aloc_ns = end_tx_aloc_ns - start_tx_aloc_ns;
	total_tx_aloc_ns = total_tx_aloc_ns + delta_tx_aloc_ns;


	struct timespec start_tx_send1_time = get_realtime();
	unsigned long start_tx_send1_ns = get_nsec(&start_tx_send1_time);
	for ( ; ; ) {
	...
	struct timespec end_tx_send1_time = get_realtime();
	unsigned long end_tx_send1_ns = get_nsec(&end_tx_send1_time);
	unsigned long delta_tx_send1_ns = end_tx_send1_ns - start_tx_send1_ns;
	total_tx_send1_ns = total_tx_send1_ns + delta_tx_send1_ns;

	struct timespec start_tx_send2_time = get_realtime();
	unsigned long start_tx_send2_ns = get_nsec(&start_tx_send2_time);
	if (xsk_ring_prod__needs_wakeup(&p->txq))
		sendto(xsk_socket__fd(p->xsk), NULL, 0, MSG_DONTWAIT, NULL, 0);
	p->n_pkts_tx += n_pkts;

	struct timespec end_tx_send2_time = get_realtime();
	unsigned long end_tx_send2_ns = get_nsec(&end_tx_send2_time);
	unsigned long delta_tx_send2_ns = end_tx_send2_ns - start_tx_send2_ns;
	total_tx_send2_ns = total_tx_send2_ns + delta_tx_send2_ns;
}
...
unsigned long total_busypoll_ns = 0;
unsigned long total_process_ns = 0;
unsigned long total_rx_ns = 0;
unsigned long total_tx_ns = 0;
// struct timespec start_time;
// unsigned long start_time_ns = 0;
...
thread_func(void *arg)
{
	...
	// start_time_ns = get_nsec(&start_time);
	...
	for (i = 0; !t->quit; i = (i + 1) & (t->n_ports_rx - 1)) {
		...
		struct timespec start_time = get_realtime();
		unsigned long start_time_ns = get_nsec(&start_time);

		// if (x != 0) {
		// 	struct timespec loop_restart_time = get_realtime();
		// 	unsigned long loop_restart_ns = get_nsec(&loop_restart_time);
		// 	total_delta_ns = loop_restart_ns - start_time_ns;
		// }
		// x++;
		...
		struct timespec busypoll_time = get_realtime();
		unsigned long busypoll_ns = get_nsec(&busypoll_time);
		unsigned long busypoll_delta_ns = busypoll_ns - start_time_ns;
		total_busypoll_ns = total_busypoll_ns + busypoll_delta_ns;

		if (!n_pkts) {
			// struct timespec end_time = get_realtime();
			// unsigned long start_time_ns = get_nsec(&start_time);
			// unsigned long end_time_ns = get_nsec(&end_time);
			// unsigned long delta_ns = end_time_ns - start_time_ns;
			// total_delta_ns = total_delta_ns + delta_ns;
			continue;
		}
		...
		for (j = 0; j < n_pkts; j++) {
			...
			struct timespec start_rx_time = get_realtime();
			unsigned long start_rx_ns = get_nsec(&start_rx_time);
			...
			struct timespec end_rx_time = get_realtime();
			unsigned long end_rx_ns = get_nsec(&end_rx_time);
			unsigned long delta_rx_ns = end_rx_ns - start_rx_ns;
			total_rx_ns = total_rx_ns + delta_rx_ns;


			struct timespec start_tx_time = get_realtime();
			unsigned long start_tx_ns = get_nsec(&start_tx_time);
			...
			struct timespec end_tx_time = get_realtime();
			unsigned long end_tx_ns = get_nsec(&end_tx_time);
			unsigned long delta_tx_ns = end_tx_ns - start_tx_ns;
			total_tx_ns = total_tx_ns + delta_tx_ns;
		}

		// unsigned long start_time_ns = get_nsec(&start_time);
		struct timespec end_process_time = get_realtime();
		unsigned long end_process_ns = get_nsec(&end_process_time);
		unsigned long delta_process_ns = end_process_ns - busypoll_ns;
		total_process_ns = total_process_ns + delta_process_ns;
	}
	...
	// unsigned long delta_ns = end_time_ns - start_time_ns;
	// total_delta_ns = total_delta_ns + delta_ns;
	// printf("HELOOOOOOOO delta_ns %ld \n", delta_ns);
	return NULL;
}
...
int main(int argc, char **argv)
{
	...
	// total_delta_ns = total_delta_ns + delta_ns;
	printf("Quit.\n");
	printf("total_busypoll_ns %ld s \n", (total_busypoll_ns/1000000000));
	printf("total_process_ns %ld s \n", (total_process_ns/1000000000));
	printf("--total_rx_ns %ld s \n", (total_rx_ns/1000000000));
	printf("--total_tx_ns %ld s \n", (total_tx_ns/1000000000)); 
	printf("----total_tx_aloc_ns %ld s \n", (total_tx_aloc_ns/1000000000)); 
	printf("----total_tx_send1_ns %ld s \n", (total_tx_send1_ns/1000000000)); 
	printf("----total_tx_send2_ns %ld s \n", (total_tx_send2_ns/1000000000));
