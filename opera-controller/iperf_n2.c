...
static int process_rx_packet(void *data, struct port_params *params, uint32_t len, u64 addr)
{
	...
	if (is_veth == 0)
	{
		...
		unsigned char out_eth_dst[ETH_ALEN+1] = { 0x9c, 0xdc, 0x71, 0x4a, 0x4c, 0xa1}; //98:f2:b3:cc:12:c1
		unsigned char out_eth_src[ETH_ALEN+1] = { 0x98, 0xf2, 0xb3, 0xcc, 0x12, 0xc1}; //9c:dc:71:4a:4c:a1
		...
		if (ntohs(eth->h_proto) != ETH_P_IP ||
			...
			{
				printf("not ETH_P_IP or size exceeds the lenth \n");
				return false;
			}
