...
static void load_xdp_program(void)
{
	...
	int veth23_ifindex = if_nametoindex("vethout23");
	int veth24_ifindex = if_nametoindex("vethout24");
	int veth25_ifindex = if_nametoindex("vethout26");
	int veth27_ifindex = if_nametoindex("vethout27");
	int veth28_ifindex = if_nametoindex("vethout28");
	int veth29_ifindex = if_nametoindex("vethout29");
	...
	struct config veth23_cfg = {
		.ifindex = veth23_ifindex,
		.ifname = "vethout23",
		.xsk_if_queue = 0,
		.xsk_poll_mode = true,
		.filename = "veth_kern.o",
	...
	struct config veth24_cfg = {
		.ifindex = veth24_ifindex,
		.ifname = "vethout24",
		.xsk_if_queue = 0,
		.xsk_poll_mode = true,
		.filename = "veth_kern.o",
	...
	struct config veth25_cfg = {
		.ifindex = veth25_ifindex,
		.ifname = "vethout26",
		.xsk_if_queue = 0,
		.xsk_poll_mode = true,
		.filename = "veth_kern.o",
	...
	struct config veth27_cfg = {
		.ifindex = veth27_ifindex,
		.ifname = "vethout27",
		.xsk_if_queue = 0,
		.xsk_poll_mode = true,
		.filename = "veth_kern.o",
	...
	struct config veth28_cfg = {
		.ifindex = veth28_ifindex,
		.ifname = "vethout28",
		.xsk_if_queue = 0,
		.xsk_poll_mode = true,
		.filename = "veth_kern.o",
	...
	struct config veth29_cfg = {
		.ifindex = veth29_ifindex,
		.ifname = "vethout29",
		.xsk_if_queue = 0,
		.xsk_poll_mode = true,
		.filename = "veth_kern.o",
	...
	struct config cfgs[9] = {nic_cfg, veth_cfg, veth3_cfg, veth23_cfg, veth24_cfg, veth25_cfg, veth27_cfg, veth28_cfg, veth29_cfg};
...
thread_func_veth_to_nic_tx(void *arg)
{
	...
	while (!t->quit)
	{
		...
		{
			while ((!ringbuf_is_empty(ring_buff_non_local[0])) && (btx_index < MAX_BURST_TX))
			{
				void *obj;
		...
		if (ring_buff[0] != NULL)
		{
			while ((!ringbuf_is_empty(ring_buff[0])) && (btx_index < MAX_BURST_TX))
			{
				...
				if (burst_tx_queue != NULL)
				{
					btx2->addr[0] = 0;
					btx2->len[0] = 0;
					ringbuf_sp_enqueue(burst_tx_queue, btx2);
				} else {
					printf("burst_tx_queue is NULL \n");
				}
...
thread_func_veth(void *arg)
{
	...
	for (x = 0; x < MAX_BURST_TX_OBJS; x++)
	{
		...
		if (burst_tx_queue != NULL)
		{
			ringbuf_sp_enqueue(burst_tx_queue, btx_obj);
			// printf("burst_tx_queue is not NULL \n");
		} else {
			printf("burst_tx_queue is NULL \n");
		}
	}
...
int main(int argc, char **argv)
{
	...
	n_ports = 9; // 0 and 1 and 2 (veth1 and nic and veth3)
	load_xdp_program();
	...
	// 	int m = d + 11;
	// 	d = d + 1;
	// 	snprintf(buf, 12, "vethout%d", m);
	// 	// port_params[i].iface = buf;
	// 	 port_params[i].iface = "veth1";
	// 	// printf("%s\n", buf);
	...
	for (i = 0; i < n_threads; i++)
	{
		...
		{							   // veth->nic (veth rx: push to local queues)
			...
			t->ports_rx[1] = ports[1]; // veth1
			t->ports_rx[2] = ports[2]; // veth3
			t->ports_rx[3] = ports[3]; // veth23
			t->ports_rx[4] = ports[4]; // veth24
			t->ports_rx[5] = ports[5]; // veth26
			t->ports_rx[6] = ports[6]; // veth27
			t->ports_rx[7] = ports[7]; // veth28
			t->ports_rx[8] = ports[8]; // veth29
			t->ring_bf_array[0] = ring_array[0];
			t->ring_bf_array[1] = ring_array[1];
			t->ring_bf_array[2] = ring_array[2];
			t->burst_tx_queue = burst_tx_queue_veth;
		}
		else if (i == 1)
